...
class SX1231 {
public:
    SX1231(SPIClass &spi, uint8_t selPin) : _spi(spi), _selPin(selPin) {}

    void begin (uint8_t id, uint8_t group, int freq);
...
void RF96fsk<SPI>::setMode (uint8_t newMode) {
    mode = newMode;
...
template< typename SPI >
void RF96fsk<SPI>::setFrequency (uint32_t hz) {
...
template< typename SPI >
void RF96fsk<SPI>::configure (const uint8_t* p) {
    while (true) {
        uint8_t cmd = p[0];
        if (cmd == 0)
            break;
...
template< typename SPI >
void RF96fsk<SPI>::init (uint8_t id, uint8_t group, int freq) {
    myId = id;
...
template< typename SPI >
void RF96fsk<SPI>::txPower (uint8_t level) {
    if (level < 2) level = 2;
...
template< typename SPI >
void RF96fsk<SPI>::send (uint8_t header, const void* ptr, int len) {
    setMode(MODE_FSTX);
